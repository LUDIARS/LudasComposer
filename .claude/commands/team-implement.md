...
## 生成プロジェクトの3フォルダ構成（重要概念）

Arsが管理するプロジェクトは必ず以下の3フォルダで構成される。エディタ・コード生成・MCP Serverのすべてがこの構造を前提に動作する。

| フォルダ | 役割 |
|---|---|
| `codedesign/` | コード設計データ（アクター構造・モジュール定義・型情報） |
| `gamedesign/` | ゲーム設計データ（シーン構成・バランス・ゲームロジック） |
| `data/` | アセット・リソースデータ（画像・音声・設定ファイル等） |

各エージェントへの指示には、担当箇所がどのフォルダと関係するかを明示すること。
