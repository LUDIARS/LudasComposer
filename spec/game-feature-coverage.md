...
## 0. サマリ (2026-05-01 更新時点 — feat/lexicon-expand-160 反映)

| 集合 | 件数 |
|------|------|
| game-lexicon に定義済 Genre | **18** (旧 9 + 新 9 = platformer / metroidvania / vampire-survivors-like / puzzle / roguelike / deckbuilder / hack-and-slash / shmup / fps) |
| game-lexicon に定義済 Preset | **18** (各ジャンル `<id>-basic`) |
| game-lexicon に定義済 Feature | **約 210** (旧 43 + 今回 +166) |
| game-template から参照される ID 総数 | **約 200** |
| そのうち lexicon に存在しないもの (C 章未追加候補) | **0** (今回の追加で全て定義済) |
...
→ 新 Feature は minimal TOML (id / 名前 / summary / tags / genres / depends_on) で
  追加。 parameter 詳細化は使う時に都度。 Ergo の lexicon カバー率は **直接
  約 2% / 部分含めて 5%** に薄まったが、 これは分母が広がっただけで Ergo 側の
  実装量は同じ。 ゲーム実装は個別 crate or プラグインで分担する前提を維持。
...
> ✅ feat/lexicon-expand-160 ですべて lexicon に取り込み済。 以下は当時の
>   未定義 ID 一覧 (履歴目的で残す)。 各 ID は対応するジャンルディレクトリに
>   minimal TOML として存在する。
...
- **Ergo 詳細実装ドキュメント (Ars 視点)**: [`spec/ergo/`](ergo/INDEX.md)
- Ergo モジュール一覧 (canonical): <https://github.com/LUDIARS/Ergo/blob/main/module_list.md>
