...
│  ├─ シーンの永続化                                   │
│  └─ アセンブリ管理 (TypeScript JIT + WASM Bundle)    │
├─────────────────────────────────────────────────────┤
│  TypeScript ゲームロジック (AI生成)                   │
│  ├─ ScriptingRuntime … JITコンパイル & ホットリロード │
│  ├─ EmscriptenBridge … C++コアエンジン↔TS連携        │
│  └─ ScriptContext    … シーン単位の実行コンテキスト   │
├─────────────────────────────────────────────────────┤
...
| カテゴリ | 管理単位 | 特殊機能 |
|---------|---------|---------|
| フォント | ファイル名 + ファミリー + スタイル | — |
| モデル | ファイル名 + ボーン構造 | ボーンパターン適合、モーションアサイン |
| テクスチャ | ファイル名 + グループ | テクスチャグループ、アトラス化 |
| モーション | ファイル名 + ボーン対象 | モーショングループ、ランダムパターン、IK/リグ設定 |
| サウンド | ファイル名 + サウンドID + カテゴリ | BGM/SE/Voice/Ambient分類 |

### 12.3 ボーンパターンシステム

モデルのボーン構造に基づくパターンマッチング:

1. **パターン定義**: 必須ボーン名リスト + オプションボーン名リスト
2. **自動検出**: モデル登録時にボーンリストからパターンを自動判定
3. **モーション適合**: 同一ボーンパターンのモーションのみアサイン可能

### 12.4 モーショングループ

- **ランダムパターン**: 重み付きランダム選択による再生
- **IK設定**: エフェクタボーン・チェーン長の共通設定
- **リグ設定**: ボーン名→コントロール名のマッピング共通化
- **ボーンパターン紐付け**: グループ全体でパターン互換性を保証

### 12.5 テクスチャグループとアトラス化

- 同一グループのテクスチャをアトラステクスチャに統合
- アトラス設定: 最大サイズ、パディング
...
## 13. TypeScriptゲームロジック & Emscriptenブリッジ設計

### 13.1 概要

ゲームロジックをTypeScriptで実装し、ホットリロードとWebGL実行に対応する。
AIによるコード生成を前提とし、TypeScriptの書きやすさ・型安全性を活用する。
C++コアエンジンとの連携はEmscriptenを採用し、WASMを介して相互呼び出しを行う。

**移行方針:**
- ゲームロジック: **C++ → TypeScript** (AI生成しやすい言語)
- コアエンジン (レンダリング等): **C++ → Emscripten → WASM** (パフォーマンス重視)
- 連携方式: **Emscriptenブリッジ** (ccall/cwrap + コールバック登録)

```
┌─────────────────────────────────────────────────────────┐
│  TypeScript ゲームロジック (AI生成)                       │
│  ├─ JITコンパイル (TypeScript → JavaScript)              │
│  ├─ ホットリロード (ファイル変更監視 → 動的差し替え)       │
│  └─ ESモジュール動的ロード (Blob URL + import())         │
├─────────────────────────────────────────────────────────┤
│  Emscripten ブリッジ                                     │
│  ├─ TypeScript → C++: cwrap/ccall 経由                   │
│  ├─ C++ → TypeScript: addFunction コールバック登録        │
│  └─ メモリ管理: malloc/free + 型変換ユーティリティ         │
├─────────────────────────────────────────────────────────┤
│  C++ コアエンジン (Emscripten → WASM)                    │
│  ├─ レンダリングエンジン (WebGL バックエンド)              │
│  ├─ 物理エンジン                                         │
│  ├─ オーディオエンジン (WebAudio バックエンド)             │
│  └─ リソースローダー                                     │
├─────────────────────────────────────────────────────────┤
│  ブラウザ / Tauri WebView                                │
│  ├─ WebGL API                                            │
...
### 13.2 TypeScriptスクリプティングランタイム

#### 組み込み (Embedding)

TypeScriptゲームロジックをブラウザ/Tauri WebView内で直接実行する仕組み:

1. **TypeScript → JavaScript トランスパイル**: TypeScript Compiler APIをブラウザ内で実行
2. **ESモジュール動的ロード**: Blob URL + `import()` による動的モジュールロード
3. **実行コンテキスト**: シーン単位で独立した実行コンテキストを管理
4. **ライフサイクル**: `init()` / `dispose()` 関数による初期化・破棄フック
...
ブラウザ内でTypeScriptをリアルタイムにコンパイル・実行:

| 項目 | 設定 |
|------|------|
| コンパイラ | TypeScript Compiler API (ブラウザ内) |
| ターゲット | ES2022 |
| モジュール | ES2020 (ES Modules) |
| ソースマップ | 有効 (デバッグ対応) |
| インクリメンタル | 有効 (変更部分のみ再コンパイル) |

#### ホットリロード

開発中のゲームロジック変更を即座に反映:

1. **ファイル変更監視**: 500ms間隔でソースコード変更を検出
2. **インクリメンタルリコンパイル**: 変更されたモジュールのみ再トランスパイル
3. **動的差し替え**: 旧モジュールの `dispose()` → 新モジュールの `init()` で安全に差し替え
4. **状態保持**: ホットリロード時にゲーム状態を可能な限り維持

### 13.3 Emscriptenブリッジ設計

#### TypeScript → C++ 呼び出し

```typescript
// cwrap でC++関数をTypeScript関数にラップ
const renderScene = bridge.call<void>('core-engine', 'renderScene', sceneId);
const getEntityPosition = bridge.call<[number, number, number]>('core-engine', 'getEntityPosition', entityId);
```

#### C++ → TypeScript コールバック

```typescript
// TypeScript関数をC++から呼び出せるよう登録
bridge.registerCallback('core-engine', {
  name: 'onCollision',
  scriptModuleId: 'game-logic',
  scriptFunctionName: 'handleCollision',
  signature: 'vii', // void(int, int)
...
| `void*` | pointer | `number` |
| `uint8_t*` | pointer | `ArrayBuffer` |

### 13.4 ビルド方式の変更

| プラットフォーム | 旧方式 | 新方式 |
|----------------|--------|--------|
| WebGL | JIT (C++) | TypeScript JIT + WASM Bundle |
| PC | DLL (C++) | TypeScript JIT + WASM Bundle |

- **TypeScript JIT**: ゲームロジックをブラウザ内でJITコンパイル・実行
- **WASM Bundle**: C++コアエンジンをEmscriptenでWASMにコンパイル

**両プラットフォーム共通**: TypeScript JITにより、WebGL/PC問わず同一コードで動作

### 13.5 アセンブリモデルの更新

```typescript
// ビルド方式の変更
type BuildMethod = 'typescript-jit' | 'wasm-bundle';
// 旧: 'jit' | 'dll'

// コアアセンブリのアーティファクト種別
type CoreArtifactType = 'source' | 'wasm';
// 旧: 'source' | 'dll'

// デフォルトビルド設定
const DEFAULT_BUILD_CONFIGS = [
  { target: 'webgl', method: 'typescript-jit' },
  { target: 'pc', method: 'typescript-jit' },
];
```
...
src/
├── types/
│   ├── scripting.ts           # スクリプティングランタイム型定義
│   │   ├── ScriptModule       # スクリプトモジュール定義
│   │   ├── JitConfig          # JITコンパイル設定
│   │   ├── HotReloadEvent     # ホットリロードイベント
│   │   └── ScriptContext      # 実行コンテキスト
│   ├── emscripten-bridge.ts   # Emscriptenブリッジ型定義
│   │   ├── EmscriptenModule   # WASMモジュール定義
│   │   ├── NativeFunction     # C++エクスポート関数
│   │   ├── CallbackBinding    # コールバックバインディング
│   │   └── WasmMemoryConfig   # WASMメモリ設定
│   └── assembly.ts            # アセンブリ型定義 (更新済み)
├── services/
│   ├── scripting-runtime.ts   # ScriptingRuntime クラス
│   │   ├── registerModule()   # モジュール登録
│   │   ├── compileModule()    # JITコンパイル
│   │   ├── loadModule()       # モジュールロード (組み込み)
│   │   ├── hotReload()        # ホットリロード
│   │   ├── createContext()    # 実行コンテキスト作成
│   │   └── startWatching()    # ファイル変更監視
│   └── emscripten-bridge.ts   # EmscriptenBridge クラス
│       ├── loadModule()       # WASMモジュールロード
│       ├── call()             # TypeScript→C++ 関数呼び出し
│       ├── registerCallback() # C++→TypeScript コールバック
│       └── allocString()      # メモリ操作ユーティリティ
└── stores/
    └── scriptingStore.ts      # スクリプティング Zustand ストア
```
...
1. **モジュール定義**: コンポーネント定義書 (Markdown) でロジック仕様を記述
2. **コード生成**: AIがTypeScriptゲームロジックを生成
3. **JITコンパイル**: 生成されたTypeScriptをブラウザ内でリアルタイムコンパイル
4. **ホットリロード**: エディタ上で即座にプレビュー反映
5. **イテレーション**: AIによる修正→ホットリロードの高速サイクル
